...
public class ProCsgoVideoSettingsController {
    ...
    @GetMapping("/getProCsgoVideoSettingsById")
    public TableResult<ProCsgoVideoSettings> getProCsgoVideoSettingsById(Integer proId) {
        ProCsgoVideoSettings proCsgoVideoSettings = proCsgoVideoSettingsService.getById(proId);
        return TableResult.ok("查询成功", proCsgoVideoSettings);
    }
    ...
    public TableResult<ProBasicInfo> getIds() {//参数名要和前端的ajax方法中的data参数里面的属性名字一致
        List<ProBasicInfo> proBasicInfoList = proCsgoVideoSettingsService.getIds();
        // getTotal()方法返回表里的总记录数,getRecords()方法返回当前页的数据列表
